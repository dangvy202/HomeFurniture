...
class Wishlist extends Component {
    ...
    componentDidMount() {
        if (
            ...
            wishlistService.getAllWishlistByEmail(sessionStorage.getItem("email"))
                .then((res) => {
    ...
    trash(product) {
        ...
        wishlistService.deleteWishlistOrder(request).then((res) => {
    ...
    reductionProduct(product, quantity) {
        if (quantity > 1) {
            ...
            wishlistService.subtractWishlist(request).then((res) => {
                this.setState({ notification: res.data.message })
            }).catch((error) => {
                this.setState({ notification: "FAIL" })
            })
                                                                    ...
                                                                    return <>
                                                                        {this.state.productList.map((item) => (
                                                                            <tr id="wishlist_1">
                                                                                ...
                                                                                <td class="wishlist_default" style={{ display: "flex", justifyContent: "space-around", height: "125px", alignItems: "center" }}>
                                                                                    <button class="button-design plus" onClick={(e) => { e.preventDefault(); this.increaseProduct(item) }}>
                                                                                        <i class="fa-solid fa-plus"></i>
                                                                                    </button>
                                                                                    <button class="button-design subtract" onClick={(e) => { e.preventDefault(); this.reductionProduct(item, item.quantity) }}>
                                                                                        <i class="fa-solid fa-minus"></i>
                                                                                    </button>
                                                                                    <button class="button-design trash" onClick={(e) => { e.preventDefault(); this.trash(item) }}>
