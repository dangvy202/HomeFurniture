...
public class OrderDetailService {
	...
	if (!CollectionUtils.isEmpty(orderDetailList)) {
	    User user = userRepository.findById(orderDetailList.stream().findFirst().get().getUser().getId())
		    .orElse(null);
	    if (user != null) {
		orderDetailList.forEach(orderDetail -> {
		    ...
		    orderProductResponse = OrderDetailMapper.INSTANCE.toOrderProductRes(orderDetail,
